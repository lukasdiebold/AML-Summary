...
\]
\begin{derivation}
Let the joint distribution be (see the Schur complement note in §\ref{sec:schur-complement})
\[
\begin{bmatrix} y \\ y_\star \end{bmatrix}
\sim \mathcal{N}\!\left(\mathbf{0},\
\begin{bmatrix}
\Sigma_{yy} & \Sigma_{y\star} \\
\Sigma_{\star y} & \Sigma_{\star\star}
\end{bmatrix}\right),\quad
\Sigma_{yy} = K + \sigma^2 I,\ \Sigma_{\star y} = k^\top,\ \Sigma_{\star\star} = c.
\]
For a jointly Gaussian vector, the conditional distribution of $y_\star$ given $y$ is Gaussian with
\[
\mathbb{E}[y_\star \mid y] = \Sigma_{\star y}\, \Sigma_{yy}^{-1} y = k^\top (K + \sigma^2 I)^{-1} y,
\]
and
\[
\operatorname{Var}(y_\star \mid y) = \Sigma_{\star\star} - \Sigma_{\star y}\, \Sigma_{yy}^{-1} \, \Sigma_{y\star}
= c - k^\top (K + \sigma^2 I)^{-1} k,
\]
which yields the stated predictive mean $\mu_\star$ and variance $\sigma_\star^2$.
\end{derivation}
