...
\end{align*}
where we used $\mathbb{E}[\beta\beta^{\top}]=\operatorname{Cov}(\beta)=\Lambda^{-1}$ for a zero-mean Gaussian. The cross terms vanish because $\mathbb{E}[\beta\varepsilon^\top]=\mathbb{E}[\beta]\mathbb{E}[\varepsilon]^\top=0$ and $\mathbb{E}[\varepsilon\beta^\top]=\mathbf{0}$ by independence and zero means.
\end{derivation}
...
\]
\end{derivation}
Hence GPs output \textbf{both a mean prediction and an uncertainty quantification}. This is however not the real uncertainty about the function value at $x_\star$, but the uncertainty we get inside our model, which is constrained to gaussian processes with a specific kernel.
